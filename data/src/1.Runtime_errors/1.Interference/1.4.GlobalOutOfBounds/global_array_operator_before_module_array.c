...
void global_array_operator_before(uint32_t runtime_0_uint, uint32_t array[]) { /* ERROR_FUNC */
  uint32_t val;

  val = array[runtime_0_uint - 1u]; /* ERROR_LINE */
  array[runtime_0_uint - 1u] = val; /* ERROR_LINE */
  (void) val;
}

int main(int runtime_1_int, char* argv[]) { (void) argv;
  module_array_Init();
  global_array_operator_before((uint32_t)runtime_1_int - 1, module_array_array); /* ERROR_FUNC */
  return 0;
}
